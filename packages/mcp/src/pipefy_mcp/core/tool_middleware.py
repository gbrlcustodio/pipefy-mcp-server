...
# Marks the installed chain handler so a second install on the same app is a
# no-op. The sentinel lives on the wrapped function (per app), not on a module or
# class global: each FastMCP instance has its own handler, so a global would wrap
# the first app and silently skip every later one built in the same process.
...
def install_tool_call_middleware(
    ...
    Idempotent per app: the sentinel lives on the installed handler, so a repeat
    install on the same app is a no-op while a different app (built later in the
    same process) wraps its own fresh handler. Raises if FastMCP has not registered
    the handler, so an SDK-internal change fails loud rather than silently skipping
    the chain.
    """
    if not middlewares:
        return
