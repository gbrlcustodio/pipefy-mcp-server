...
- [ ] `create_pipe` creates a pipe and returns its ID
- [ ] `update_pipe` updates pipe attributes (name, icon, color)
- [ ] `delete_pipe` returns a preview by default; deletes only when `confirm=true`
- [ ] `clone_pipe` clones a pipe synchronously and returns the new pipe with ID
- [ ] `create_phase` creates a phase in a pipe with name, done flag, index
- [ ] `update_phase` / `delete_phase` work on existing phases
- [ ] `create_phase_field` creates a field with pass-through type validation
- [ ] `update_phase_field` / `delete_phase_field` work on existing fields
- [ ] `create_label` / `update_label` / `delete_label` manage labels on a pipe
- [ ] All `delete_*` tools have `destructiveHint=True` in `ToolAnnotations`
...
- [ ] **1.1** Create pipe_config_queries.py with pipe mutations
...
- [ ] **2.1** Write tests for PipeConfigService pipe methods
...
- [ ] **2.2** Implement pipe methods in PipeConfigService
...
- [ ] **2.3** Add pipe methods to PipefyClient facade
...
- [ ] **2.4** Create pipe_config_tool_helpers.py
...
- [ ] **2.5** Write tests for pipe tools
...
- [ ] **3.1** Write tests for phase service methods
...
- [ ] **3.2** Implement phase methods in PipeConfigService
...
- [ ] **3.3** Add phase methods to PipefyClient facade
  - **File**: `src/pipefy_mcp/services/pipefy/client.py` (modify)
...
- [ ] **4.1** Write tests for field service methods
  - **File**: `tests/services/pipefy/test_pipe_config_service.py` (modify)
...
- [ ] **4.2** Implement field methods in PipeConfigService
  - **File**: `src/pipefy_mcp/services/pipefy/pipe_config_service.py` (modify)
...
- [ ] **4.6** Write tests for field + label tools
  - **File**: `tests/tools/test_pipe_config_tools.py` (modify)
  - **What**: Add test cases for all 6 tools: 3 field tools + 3 label tools.
  - **Verify**: Tests fail (red).

- [ ] **4.7** Implement field + label tools
  - **File**: `src/pipefy_mcp/tools/pipe_config_tools.py` (modify)
...
- [ ] **5.1** Register PipeConfigTools in ToolRegistry
...
- [ ] **5.2** Export PipeConfigService from package
...
- [ ] **5.3** Update README with pipe building tools
...
- [ ] All 13 tools callable via MCP Inspector
- [ ] `uv run pytest` — full test suite passes
- [ ] `uv run pytest --cov=src/pipefy_mcp/services/pipefy --cov-report=term-missing` — ≥95% coverage on PipeConfigService
- [ ] `uv run ruff check src/ && uv run ruff format src/` — no lint errors
- [ ] `delete_pipe` follows two-step preview/confirm pattern
- [ ] All `delete_*` tools have `destructiveHint=True`
- [ ] README documents all 13 new tools
