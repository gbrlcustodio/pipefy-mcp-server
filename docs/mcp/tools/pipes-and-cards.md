...
| `upload_attachment_to_card` | Presigned URL + S3 PUT + `updateCardField` for **attachment** fields. **One file per call**: to attach multiple files, call the tool once per file. Exactly one of `file_path` (local filesystem path the MCP server reads; supports `~` expansion) or `file_content_base64` (in-memory bytes the agent generated). When `file_path` is provided, `file_name` is inferred from the path's basename if omitted; with `file_content_base64`, `file_name` is required. Optional `content_type` is inferred from `file_name` when omitted. Files larger than **100 MiB** are rejected locally before any network call. **`field_id` must be the field slug** (e.g. `document_upload`), not the uuid: using the uuid returns `RESOURCE_NOT_FOUND`. |
