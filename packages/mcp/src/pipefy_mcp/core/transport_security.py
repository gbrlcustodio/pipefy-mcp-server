...
The SDK auto-enables a loopback-only Host/Origin allowlist whenever the transport is
built with a loopback ``host`` and no explicit ``transport_security``. The serving path
always passes ``host=settings.mcp.host`` (default ``127.0.0.1``), so behind a proxy that
...
# server over loopback). Mirrors the SDK's own loopback auto-enable set.
...
def build_transport_security(
    ...
    """Resolve the transport allowlist, or ``None`` to keep the SDK's default.

    Returns ``None`` only when there is nothing to configure: no ``resource``, no
    ``allowed_hosts``, and an unset (``None``) ``allowed_origins``, so the SDK applies
...
__all__ = ["build_transport_security", "transport_security_for"]
