...
- **MCP**: attachment uploads accept `file_path` (local filesystem path, with `~` expansion) instead of `file_url`. The MCP server runs as a local subprocess of the user's agent runtime, so the SSRF defenses that defended a hosted URL fetch (`_validate_url_safe`, redirect cap, 100 MiB size cap) are no longer carried; `file_content_base64` remains as a fallback for in-memory bytes the agent never wrote to disk. `file_name` is optional when `file_path` is provided (basename inferred). `PIPEFY_ALLOW_INSECURE_URLS` continues to gate `base_url` / `auth_url` policy but no longer affects attachments. See `packages/mcp/AGENTS.md` for the `GATED:SELF_HOSTED` convention that marks where URL ingestion would return under a future self-hosted profile.
