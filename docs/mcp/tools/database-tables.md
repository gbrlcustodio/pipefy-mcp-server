...
| **Record CRUD** | `create_table_record`, `update_table_record`, `delete_table_record`, `set_table_record_field_value`, `upload_attachment_to_table_record` | `upload_attachment_to_table_record` runs presigned URL + S3 PUT + `setTableRecordFieldValue` for attachment columns. **One file per call**: to attach multiple files, call the tool once per file. Same inputs as card upload: exactly one of `file_path` (local, MCP server reads it; local profile only) or `file_url` (downloaded under an SSRF guard; any profile). On the hosted server `file_path` is rejected — pass `file_url`. `file_name` is inferred from the source basename when omitted; either source is rejected above **100 MiB**. **`field_id` must be the field slug**, not the uuid. |
