...
- [ ] `introspect_type` returns fields/inputFields/enumValues for any valid GraphQL type
- [ ] `introspect_mutation` returns name, description, args, and return type for any mutation
- [ ] `search_schema` returns matching types by keyword (case-insensitive)
- [ ] `execute_graphql` executes arbitrary queries/mutations and returns raw JSON
- [ ] `execute_graphql` validates syntax via `gql()` before sending
- [ ] `execute_graphql` surfaces GraphQL errors clearly (not swallowed)
- [ ] Introspection tools have `readOnlyHint=True`; `execute_graphql` has `readOnlyHint=False`
- [ ] All tools target the standard GraphQL endpoint only

---

## Task 1: Define introspection GraphQL queries

- [ ] **1.1** Create introspection_queries.py with type introspection query
...
- [ ] **1.2** Add mutation introspection query
...
- [ ] **1.3** Add schema types listing query
  - **File**: `src/pipefy_mcp/services/pipefy/queries/introspection_queries.py` (modify)
...
- [ ] **2.1** Write tests for introspect_type
...
- [ ] **2.2** Implement introspect_type method
...
- [ ] **2.3** Write tests for introspect_mutation
...
- [ ] **2.4** Implement introspect_mutation method
...
- [ ] **2.5** Write tests for search_schema
  - **File**: `tests/services/pipefy/test_schema_introspection_service.py` (modify)
...
- [ ] **2.6** Implement search_schema method
  - **File**: `src/pipefy_mcp/services/pipefy/schema_introspection_service.py` (modify)
...
- [ ] **2.7** Write tests for execute_graphql
  - **File**: `tests/services/pipefy/test_schema_introspection_service.py` (modify)
...
- [ ] **2.8** Implement execute_graphql method
  - **File**: `src/pipefy_mcp/services/pipefy/schema_introspection_service.py` (modify)
...
- [ ] **3.1** Add SchemaIntrospectionService to PipefyClient
...
- [ ] **3.2** Export service from package
...
- [ ] **4.1** Create introspection_tool_helpers.py
...
- [ ] **4.2** Write tests for introspection tools
...
- [ ] **4.3** Implement introspect_type tool
...
- [ ] **4.4** Implement introspect_mutation tool
...
- [ ] **4.5** Implement search_schema tool
  - **File**: `src/pipefy_mcp/tools/introspection_tools.py` (modify)
...
- [ ] **4.6** Implement execute_graphql tool
  - **File**: `src/pipefy_mcp/tools/introspection_tools.py` (modify)
...
- [ ] **5.1** Register IntrospectionTools in ToolRegistry
...
- [ ] **5.2** Update README with introspection tools
...
- [ ] All 4 tools callable via MCP Inspector: `introspect_type`, `introspect_mutation`, `search_schema`, `execute_graphql`
- [ ] `uv run pytest` — full test suite passes
- [ ] `uv run pytest --cov=src/pipefy_mcp/services/pipefy --cov-report=term-missing` — ≥95% coverage on new service
- [ ] `uv run ruff check src/ && uv run ruff format src/` — no lint errors
- [ ] README documents all 4 new tools
