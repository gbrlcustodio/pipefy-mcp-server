...
The SDK, the MCP server, and the CLI all touch the same domain. Without a stated rule, multi-step logic and product decisions drifted into whichever layer was open. A hidden `get_pipe` inside an SDK method is the symptom: orchestration the caller cannot see, living below the layer that owns the intent. A second question was left open: when may identifier resolution live in the SDK at all?
