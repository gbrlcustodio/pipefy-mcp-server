...
- **MCP / CLI**: attachment uploads accept `file_path` only (local filesystem path, with `~` expansion). `file_url` and `file_content_base64` are both gone — the MCP server runs as a local subprocess of the user's agent runtime, so any path the user can read is a valid source, and agents that hold generated bytes write them to a temp file and pass the path. `file_name` is optional (basename inferred from the path). The 100 MiB size cap is enforced as a local pre-flight in `pipefy_infra.filesystem.LocalFile(max_size_bytes=…)` on both surfaces, and an SDK-side backstop in `attachment_upload` catches direct-SDK callers that bypass the surface. `PIPEFY_ALLOW_INSECURE_URLS` continues to gate `base_url` / `auth_url` policy but no longer affects attachments. New SDK domain objects: `pipefy_infra.filesystem.LocalFile` (read pipeline + `LocalFileError`) and `pipefy_sdk.Attachment` (binds a `LocalFile` to a target field, runs the upload, carries the result). See `packages/mcp/AGENTS.md` for the `GATED:SELF_HOSTED` convention that marks where URL ingestion would return under a future self-hosted profile.
