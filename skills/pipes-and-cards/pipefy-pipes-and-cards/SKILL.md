...
| `update_card` | `pipefy card update <id>` (`--field-updates` = `updateFieldsValues`) | No | Update title, assignee, due date, fields. For connector/multi-value, prefer `--field-updates` / `field_updates` with `operation` ADD/REMOVE and related **card ids**. |
| `update_card_field` | *(MCP only; no direct CLI twin — use `card update --field-updates`)* | No | Single-field `updateCardField`; connector/multi-value is **replace-all** of related **card ids** (not display titles). Do not rebuild the list from `get_card` `value` (titles only). Prefer `update_card` + ADD/REMOVE instead. |
...
- **Connections missing after a connector field update:** `update_card_field` is replace-all — writing one related card id drops the rest. Prefer `update_card` / CLI `card update --field-updates` with `operation` ADD/REMOVE and related **card ids**. Do not rebuild a full list from `get_card` `value` (display titles only, not ids). For links via a pipe relation (not a connector field), use `create_card_relation` / `delete_card_relation` — see `skills/relations/`.
