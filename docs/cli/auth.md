...
> **Removed legacy names:** `PIPEFY_OAUTH_CLIENT` and `PIPEFY_OAUTH_SECRET` are removed outright (no alias, no deprecation warning) and are ignored if set; use `PIPEFY_SERVICE_ACCOUNT_CLIENT_ID` / `PIPEFY_SERVICE_ACCOUNT_CLIENT_SECRET`. `PIPEFY_OAUTH_URL` has no replacement: the OAuth token endpoint derives from `PIPEFY_BASE_URL`. See [`docs/MIGRATION.md`](../MIGRATION.md#service-account-env-var-rename).
...
Production: `https://signin.pipefy.com/realms/pipefy` (the `PIPEFY_AUTH_ISSUER_URL` default). For a non-prod IdP, set `PIPEFY_AUTH_ISSUER_URL` to the full issuer URL (host + realm) directly. The CLI does not try to derive it from any tenant convention.
...
1. Read `PIPEFY_AUTH_ISSUER_URL` (issuer) and `PIPEFY_AUTH_CLIENT_ID` (default `pipefy-cli`).
