...
A destructive operation carries the same split. `QR-6` asks that the operation name what it affects before it runs, and `QR-3` asks that no run block on an answer when no human is present. Together they leave the choice to that same ability, exactly as ambiguity does above.

The path that holds today does not read that ability, and it never did. A destructive tool previews what it affects, and it acts only after an explicit confirmation from the caller. One explicit answer therefore serves the interactive case and the ambient case alike, and [Known gaps](#known-gaps) carries the design question. [`packages/mcp/AGENTS.md`](../../packages/mcp/AGENTS.md) owns the protocol, and it records two limits. The guard protects against accident and not against intent, because a caller can confirm without a preview. Authorization stays the API's.
...
- `QR-3` holds when no human is present, so neither a consent dialog nor a question about a missing input can run. What `QR-6` would ask for becomes an explicit flag, and a run that has no flag fails rather than guessing. `QR-22` does not hold that way, and [Known gaps](#known-gaps) states what happens instead.
...
- The caller-aware destructive path, which is `QR-6`. One explicit answer serves the interactive case and the ambient case alike, so nothing about the caller decides anything here. Elicitation is the candidate for the interactive half. It cannot carry the authorization half, because a client can auto-accept an elicitation prompt when a tool runs programmatically. The artifact is a destructive path that reads whether the caller can be asked and keeps the explicit answer as the only ambient authorization.
- `QR-22` holds for some callers and not others. `create_card` and `fill_card_phase_fields` ask through `ctx.elicit` where `supports_elicitation` in `packages/mcp/src/pipefy_mcp/tools/mcp_capabilities.py` passes, which takes a declared capability and a request that can carry a server-initiated one. The client decides the second, through the revision it negotiates and the transport shape it opens, and the hosted wrapper never passes. Where the check fails, both tools proceed with the fields they were given and say nothing about it in the answer. [`docs/mcp/tools/pipes-and-cards.md`](../mcp/tools/pipes-and-cards.md) states that fallback and names who gets it, and it records that an unmatched key is dropped without an error, so a caller who supplied a bad field id learns nothing either way. Revision 2026-07-28 replaces the mechanism rather than removing it, and the replacement is a different shape: a tool returns an `InputRequiredResult` naming what it needs, the client retries, and the tool runs again from the top and reads `ctx.input_responses`. The pinned `mcp` release carries it, and `packages/mcp/src/pipefy_mcp/core/tool_middleware.py` already treats such a result as a success. No tool returns one. The artifact is a resumable tool, which costs the state that `create_card` holds across its `await` today, and `ctx.protocol_version` is what lets one tool serve both eras.
