...
- **SDK / MCP / CLI**: `get_pipe` selects `cards_count` on each workflow phase in `phases[]` (start form excluded from `phases`; use `startFormPhaseId` + `get_phase` / `get_phase_cards` for start-form inventory).
