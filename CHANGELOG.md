...
- **SDK / MCP / CLI (automation `--extra`)**: automation input keys are normalized to the spelling the API requires before the mutation, so callers may pass either case. Top-level `CreateAutomationInput` / `UpdateAutomationInput` keys (`action_params`, `event_params`, ...) and the immediate keys of `action_params` / `event_params` are mapped to the API's casing, which mixes `to_phase_id` with `httpMethod` / `fromPhaseId`; deeper payloads (`taskParams`, `aiParams`, `field_map`) are left untouched. CLI `automation create` adds flags that build these envelopes without hand-written JSON: `--to-phase`, `--trigger-phase`, `--from-phase`, `--in-phase`, `--trigger-fields`, `--email-template`, `--url`, `--http-method`, `--request-body`, `--headers`; each rejects a run that also sets the matching `action_params`/`event_params` block in `--extra`.
