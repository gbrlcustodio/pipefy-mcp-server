...
- **MCP**: the attachment-download SSRF gate (`_validate_url_safe`) is now HTTPS-only by default and respects `PIPEFY_ALLOW_INSECURE_URLS`, matching the policy switch on `base_url` and `auth_url`. Operators who depended on plain-http attachment fetches (local development against a non-TLS storage host) must set `PIPEFY_ALLOW_INSECURE_URLS=true`, or supply the file via `file_content_base64` instead. Previously, the attachment surface hard-coded http acceptance regardless of the policy.
...
- **MCP**: `pipefy_mcp.tools.attachment_tools._validate_url_safe` now delegates to `pipefy_infra.security.validate_and_assert_public_url`, which bundles the sync gate (scheme + literal-IP) and the async DNS gate in one call. The scheme check honours `PIPEFY_ALLOW_INSECURE_URLS` (see the Changed section); the SSRF defense is now greppable through the single security namespace.
