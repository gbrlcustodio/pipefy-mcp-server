...
The token binds to one tool, one resource identity, and one caller. It is HMAC-signed and replayable within a 300-second TTL; the server does not store it. Hosted verify is stateless HMAC derived from the bearer, so any replica can check the same confirmation token.
...
`execute_graphql` queries stay ungated. Mutations need the same confirmation token. Tokens are replayable, so a non-idempotent mutation can run twice if resent within the TTL. Prefer dedicated tools for those writes.
