...
- **Auth / SDK**: the per-module `_URL_SHAPE_PATTERN` regex and `_strip_str` field-validator bodies are now sourced from `pipefy_infra` (`security.URL_SHAPE_PATTERN`, `pipefy_infra.strings.strip_str`); the previous private copies in `pipefy_auth.settings` and `pipefy_sdk.settings` are gone. Behaviour is unchanged; both models still register their own `_strip_str` classmethod against their own field tuples, but the actual strip semantics live in one place.
- **Infra**: `pipefy_infra` no longer re-exports symbols at the package root; the root exposes only `__version__`. Submodules: `pipefy_infra.config` (Pipefy on-disk configuration: path discovery + TOML source), `pipefy_infra.security` (SSRF defenses: shape pattern + internal-IP gate + DNS-rebinding gate), `pipefy_infra.strings` (generic string helpers; current sole inhabitant is `strip_str`). The `security` module is the SSRF audit namespace: import the module and call through it so every call site is greppable via `security.` (same idiom as stdlib `hmac.compare_digest`, `secrets.token_urlsafe`).
- **MCP**: `pipefy_mcp.tools.attachment_tools._validate_url_safe` now delegates DNS gating to `pipefy_infra.security.assert_hostname_resolves_to_public_ips` instead of carrying its own private `_PRIVATE_NETWORKS` allowlist. The scheme check (which accepts both http and https for attachment downloads) stays inline. Behaviour is unchanged; the SSRF defense is now greppable through the single security namespace.
