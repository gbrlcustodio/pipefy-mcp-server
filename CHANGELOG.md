...
- **SDK**: `PipefySettings.graphql_url` now defaults to `https://app.pipefy.com/graphql` (Pipefy production endpoint) when `PIPEFY_GRAPHQL_URL` is unset — removes the previous `PIPEFY_GRAPHQL_URL is required` exit-2 friction in CLI commands and lets plugin / installer one-shot users skip the env var entirely. Override by setting `PIPEFY_GRAPHQL_URL=<url>` to a non-prod tenant; explicit-empty `PIPEFY_GRAPHQL_URL=` keeps the missing-URL exit for hosts that need an explicit value. `DEFAULT_GRAPHQL_URL` is re-exported from `pipefy_sdk` for plugin / installer fallbacks. Closes #238.
