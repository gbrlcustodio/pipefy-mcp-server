...
def _build(*, resource_server_url=None, allowed_hosts=None, allowed_origins=None):
    """Build the allowlist the way the runtime does: parse the URL once, feed it in."""
    resource = (
        ResourceServer.from_url(resource_server_url) if resource_server_url else None
    )
    mcp = McpSettings(allowed_hosts=allowed_hosts, allowed_origins=allowed_origins)
    return build_transport_security(mcp, resource)


@pytest.mark.unit
def test_unset_returns_none_to_keep_sdk_default():
...
def test_resource_server_url_host_is_derived_and_protection_enabled():
    ...
    security = _build(resource_server_url="https://mcp.pipefy.com/mcp")
...
def test_resource_server_url_with_explicit_port_keeps_host_and_hostport():
    ...
    security = _build(resource_server_url="https://mcp.pipefy.com:8443/mcp")
    assert security is not None
    assert "mcp.pipefy.com" in security.allowed_hosts
...
def test_ipv6_literal_resource_url_is_bracketed_to_match_the_wire_host():
    ...
    """
    security = _build(resource_server_url="https://[2606:4700:4700::1111]/mcp")
    assert security is not None
...
def test_ipv6_literal_with_port_keeps_bracketed_host_and_hostport():
    ...
    security = _build(resource_server_url="https://[2606:4700:4700::1111]:8443/mcp")
    assert security is not None
    assert "[2606:4700:4700::1111]" in security.allowed_hosts
...
def test_explicit_allowed_hosts_alone_enable_protection():
    ...
    security = _build(allowed_hosts=["proxy.internal"])
    assert security is not None
    assert security.enable_dns_rebinding_protection is True
    assert "proxy.internal" in security.allowed_hosts
...
def test_explicit_empty_origins_alone_still_enable_protection():
    """Empty origins with no host still builds an allowlist (not the SDK's default)."""
