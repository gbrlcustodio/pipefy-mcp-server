...
- `uv sync` installs dependencies using `uv` (recommended in this repo). Requires Python 3.12+ (see `.python-version`).
- CI runs on GitLab (`.gitlab-ci.yml`): ruff on changed Python files for merge requests; full test suite on each pipeline.
- `uv run pipeclaw` runs the MCP server locally via the project script.
...
- `npx @modelcontextprotocol/inspector uv --directory . run pipeclaw` launches the MCP Inspector for manual tool testing.

## Coding Style & Naming Conventions
- Python 3.12+ code lives under `src/` and follows standard module naming (`snake_case` files, `PascalCase` classes).
