...
`execute_graphql` queries stay ungated. Mutations need the same confirmation token, and the token binds to the document and the variables, so changing either one invalidates it. Tokens are replayable, so a non-idempotent mutation can run twice if resent within the TTL. Prefer dedicated tools for those writes.
...
The iPaaS token binds to the whole `arguments` map, not only the operation, so step 2 must resend `arguments` byte-for-byte as step 1 sent it. The preview echoes that map in `resource` and `message`, so anything passed as an argument, including a value that identifies a customer record, appears in the transcript the caller shows for approval.
