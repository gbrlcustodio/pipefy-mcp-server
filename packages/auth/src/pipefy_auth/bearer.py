...
class RefreshableBearerAuth(Auth):
    ...
    Pairs per-request token rotation (the provider is re-called on every request,
    so a keychain-backed refresh in :func:`ensure_fresh_session` is observed
    without rebuilding the SDK client) with a 401 safety net: when an API call
    returns 401, ``force_refresh`` is
