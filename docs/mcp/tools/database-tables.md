...
| **Record CRUD** | `create_table_record`, `update_table_record`, `delete_table_record`, `set_table_record_field_value`, `upload_attachment_to_table_record` | `upload_attachment_to_table_record` runs presigned URL + S3 PUT + `setTableRecordFieldValue` for attachment columns. **One file per call**: to attach multiple files, call the tool once per file. Same inputs as card upload: one of `file_path` (local filesystem path the MCP server reads; supports `~` expansion) or `file_content_base64` (in-memory bytes). `file_name` is inferred from the path's basename when `file_path` is provided and `file_name` is omitted; required for the base64 mode. Files larger than **100 MiB** are rejected locally before any network call. **`field_id` must be the field slug**, not the uuid. |
