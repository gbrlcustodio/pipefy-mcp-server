...
5. Submit or refresh the Cursor Marketplace listing, from `main`, once the tag is published. The listing is not cut by any workflow: `.cursor-plugin/plugin.json` and `mcp.json` ship with the repository, and a human submits them at [cursor.com/marketplace/publish](https://cursor.com/marketplace/publish). Until that submission lands, README section 6, `docs/uninstall.md`, and the onboarding skill all say the listing publishes from `main`, so this step is what makes those sentences true. Cursor reviews every plugin by hand and re-reviews each update, so budget for a review round rather than treating the submission as instant.
