...
- **`docs/contributing/architecture.md`**. The map of the code. The quality goals and the constraints that shape it, the boundary with its diagram, and the decomposition into applications, packages, and layers. Then one runtime scenario, the rules that cross every package, the known debt, and the glossary.
