...
This split covers the three surfaces (SDK, CLI, MCP server) and the shared support libraries (`auth`, `infra`). A shared library is not a surface. It owns no driving port. It adds a driven port only where there is payoff, so a pure-utility library holds none.
