...
For install and per-client MCP wiring (Cursor, Claude Desktop, Claude Code, Codex), see the root [`README.md#installation`](../../README.md#installation). For environment variables and `config.toml`, see [`../config.md`](../config.md). Edge cases (`errSecInvalidOwnerEdit`, `.mcp.json`, local-clone alternative): [`packages/mcp/README.md`](../../packages/mcp/README.md).
