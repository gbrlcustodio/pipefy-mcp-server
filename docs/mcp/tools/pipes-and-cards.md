...
**Choosing card updates:** `update_card` + `field_updates` = several custom fields at once (use `operation` ADD/REMOVE for connector/multi-value connection fields — preferred). `update_card_field` = one field, full replacement — for connectors that means the **entire** related-card **id** list; `get_card` field `value` is display titles only, so do not rebuild the list from it. `update_card` with attribute args = metadata (combinable with `field_updates`).
