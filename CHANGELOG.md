...
- **MCP / CLI**: attachment uploads accept `file_path` (local filesystem path, with `~` expansion) instead of `file_url`. The MCP server runs as a local subprocess of the user's agent runtime, so the SSRF defenses that defended a hosted URL fetch (`_validate_url_safe`, redirect cap) are no longer carried; `file_content_base64` remains as a fallback for in-memory bytes the agent never wrote to disk. `file_name` is optional when `file_path` is provided (basename inferred). The 100 MiB size cap is retained as a local pre-flight check via `pipefy_sdk.assert_attachment_size_within_cap`: oversized files fail before any network call, on both surfaces. `PIPEFY_ALLOW_INSECURE_URLS` continues to gate `base_url` / `auth_url` policy but no longer affects attachments. See `packages/mcp/AGENTS.md` for the `GATED:SELF_HOSTED` convention that marks where URL ingestion would return under a future self-hosted profile.
...
- **MCP**: dropped URL ingestion from attachment uploads (`file_url` parameter, `_validate_url_safe` helper, redirect loop, 100 MiB download cap). The MCP server runs as a local subprocess of the agent runtime, so attachments now come from `file_path` (local filesystem) or `file_content_base64` (in-memory bytes). The SSRF / size-cap defenses are recorded as `GATED:SELF_HOSTED` in `packages/mcp/src/pipefy_mcp/tools/attachment_tools.py`; bring them back behind a capability flag if a hosted MCP profile is added. The shared `pipefy_infra.security.validate_and_assert_public_url` helper stays, used by `base_url` / `auth_url` validation. See `packages/mcp/AGENTS.md` for the `GATED:<PROFILE>` convention.
