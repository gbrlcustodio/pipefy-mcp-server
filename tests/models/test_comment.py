...
def test_update_comment_input_accepts_text_at_max_length_boundary():
    ...
    text = "a" * MAX_COMMENT_TEXT_LENGTH
    payload = UpdateCommentInput(comment_id=1, text=text)
...
def test_update_comment_input_valid_input():
    ...
    payload = UpdateCommentInput(comment_id=456, text="Updated message")
    assert payload.comment_id == 456
    assert payload.text == "Updated message"
...
def test_delete_comment_input_valid_input():
    ...
    payload = DeleteCommentInput(comment_id=789)
    assert payload.comment_id == 789
