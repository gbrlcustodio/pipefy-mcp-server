...
def test_without_fix_does_not_rewrite_a_missing_skill(monkeypatch, tmp_path):
    extra = "skills/observability/pipefy-observability"
    _write_plugin(tmp_path)
    extra_dir = tmp_path / extra
    extra_dir.mkdir(parents=True)
    ...
    monkeypatch.setattr(
        _lint,
        "_tracked_skill_md_paths",
        lambda root: [_SKILL_MD, f"{extra}/SKILL.md"],
    )
    assert _lint.main() == 1
    assert plugin.read_bytes() == before


def test_fix_writes_missing_skill_and_then_passes(monkeypatch, tmp_path):
    extra = "skills/observability/pipefy-observability"
    _write_plugin(tmp_path)
    extra_dir = tmp_path / extra
    extra_dir.mkdir(parents=True)
    (extra_dir / "SKILL.md").write_text("# extra\n", encoding="utf-8")
    listing = [_SKILL_MD, f"{extra}/SKILL.md"]
    monkeypatch.setattr(_lint, "REPO_ROOT", tmp_path)
    monkeypatch.setattr(_lint, "_tracked_skill_md_paths", lambda root: listing)
    assert _lint.main(["--fix"]) == 0
    manifest = json.loads(
        (tmp_path / ".cursor-plugin" / "plugin.json").read_text(encoding="utf-8")
    )
    assert f"./{extra}" in manifest["skills"]
    assert f"./{_SKILL}" in manifest["skills"]
    assert _lint.collect_errors(tmp_path, listing) == []


def test_fix_drops_stale_manifest_entry(monkeypatch, tmp_path):
    stale = "skills/does-not-exist/fake-skill"
    _write_plugin(tmp_path, skills=[f"./{_SKILL}", f"./{stale}"])
    listing = [_SKILL_MD]
    monkeypatch.setattr(_lint, "REPO_ROOT", tmp_path)
    ...
    assert all(stale not in path for path in manifest["skills"])
    assert f"./{_SKILL}" in manifest["skills"]
    assert _lint.collect_errors(tmp_path, listing) == []


def test_fix_is_noop_when_already_aligned(monkeypatch, tmp_path):
    _write_plugin(tmp_path)
    plugin = tmp_path / ".cursor-plugin" / "plugin.json"
    before = plugin.read_bytes()
    monkeypatch.setattr(_lint, "REPO_ROOT", tmp_path)
...
def test_fix_does_not_echo_planted_mcp_values(monkeypatch, tmp_path, capsys):
    extra = "skills/observability/pipefy-observability"
    _write_plugin(
        tmp_path,
        mcp={
    ...
    assert _lint.main(["--fix"]) == 1
    err = capsys.readouterr().err
    _assert_redacted_field_failure([err], "url")
