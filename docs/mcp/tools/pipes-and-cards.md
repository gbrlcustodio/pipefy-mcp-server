...
**Choosing card updates:** `update_card_field` = one field, full replacement — for connector/multi-value connection fields that means the **entire** related list is replaced (`get_card(include_fields=true)` → merge → write, or use `update_card` + `field_updates` with `operation` ADD/REMOVE). `update_card` + `field_updates` = several custom fields at once. `update_card` with attribute args = metadata (combinable with `field_updates`).
