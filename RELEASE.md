...
- **A red DCO check on a back-merge is expected, and does not block it.** A squash merge rewrites the author to the merging account's GitHub profile identity while the `Signed-off-by` trailer, being message text, is carried verbatim. A contributor whose git `user.name` differs from their GitHub display name therefore lands a commit on `main` whose author no longer matches its own sign-off, even though it passed DCO on its own pull request. That commit cannot be rewritten once it is on the default branch, so `dco-check` fails on every back-merge that carries it. The generated pull request body names the offending commits so the failure does not read as a new problem, and no ruleset requires the check.
