...
async def test_add_card_comment_tool_can_be_called_via_in_memory_mcp_session():
    ...
    previous_client = container.pipefy_client
    container.pipefy_client = _FakePipefyClient()  # type: ignore[assignment]

    try:
        app = FastMCP("test-pipefy-mcp")
        PipeTools.register(app)
        ...
        container.pipefy_client = previous_client
