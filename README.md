...
| `get_card_inbox_emails` | Yes | List emails (sent and received) for a card's inbox. Use **`email_type: 'received'`** to get only replies (e.g. when someone replies to an email sent from the card). |
...
**Eight tools** for monitoring AI agent and automation execution, usage stats, credit consumption, and exporting job history. All read tools use `readOnlyHint=True`; the export mutation does not.
