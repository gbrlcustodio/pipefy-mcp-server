...
On macOS, `pipefy auth login` may exit with `errSecParam (-25244)` at the keychain write step when invoked from a non-TTY subprocess (e.g. the Claude Code Bash tool) because `Security.framework` cannot surface the new-item ACL dialog. The OAuth handshake itself completes; only persistence stumbles. Run `pipefy auth login` once from a regular Terminal.app session to clear the ACL prompt, then subsequent slash-command invocations work. Issue #235 tracks platform-aware error messaging.
