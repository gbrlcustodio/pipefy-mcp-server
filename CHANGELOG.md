...
- **Auth / CLI / MCP (breaking)**: the interactive-login and keychain env vars are namespaced under `PIPEFY_AUTH_*`. `PIPEFY_AUTH_URL` → `PIPEFY_AUTH_ISSUER_URL` (field and TOML key now `issuer_url`, was `auth_url`); `PIPEFY_AUTH_CLIENT_ID` keeps its env name but its field and TOML key are now `client_id` (was `auth_client_id`); `PIPEFY_DISABLE_STORED_SESSION` → `PIPEFY_AUTH_DISABLE_STORED_SESSION` (TOML key `disable_stored_session` unchanged); `PIPEFY_KEYCHAIN_BACKEND` → `PIPEFY_AUTH_KEYCHAIN_BACKEND` (TOML key `keychain_backend` unchanged). `AuthSettings` uses `extra="ignore"`, so a stale old name is dropped silently (no exception, no warning) and the default applies. Audit `.env`, MCP client JSON, and CI secrets before upgrading. `PIPEFY_TOKEN`, `PIPEFY_SERVICE_ACCOUNT_CLIENT_ID` / `PIPEFY_SERVICE_ACCOUNT_CLIENT_SECRET`, `PIPEFY_BASE_URL`, and `PIPEFY_ALLOW_INSECURE_URLS` are unchanged. See [`docs/MIGRATION.md`](docs/MIGRATION.md).
...
- **Auth / SDK (breaking)**: the legacy `PIPEFY_OAUTH_CLIENT` / `PIPEFY_OAUTH_SECRET` env-var aliases and the one-shot stderr deprecation-warning shim are removed outright, with no grace period. The names are ignored if set (no alias, no warning); use `PIPEFY_SERVICE_ACCOUNT_CLIENT_ID` / `PIPEFY_SERVICE_ACCOUNT_CLIENT_SECRET`. `PIPEFY_OAUTH_URL` remains removed without a replacement (the OAuth token endpoint derives from `PIPEFY_BASE_URL`). See [`docs/MIGRATION.md#service-account-env-var-rename`](docs/MIGRATION.md#service-account-env-var-rename).
