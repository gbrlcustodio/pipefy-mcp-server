...
5. Before opening a PR here: add the skill to `.cursor-plugin/plugin.json` (`skills` array), run `uv run python .github/workflows/scripts/lint_skill_refs.py` and `python3 .github/workflows/scripts/lint_cursor_plugin.py`, and use `git commit -s` ([DCO](../../CONTRIBUTING.md)).
