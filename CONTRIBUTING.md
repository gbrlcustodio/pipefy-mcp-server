...
4. Stage new skill files first (`git add`). Pre-commit runs
   `lint_cursor_plugin.py --fix`, which rewrites the `skills` array in
   `.cursor-plugin/plugin.json` from tracked `SKILL.md` files (excluding
   `skills/_template/`). If the hook rewrites the manifest, the commit is
   aborted so you can re-stage it — the same pattern as `ruff --fix`. CI
   still runs the lint without `--fix`. `git ls-files` only sees
   tracked/staged files, so an unstaged skill looks green locally until you
   `git add`.

   ```bash
   git add skills/<domain>/pipefy-<domain>-<action>
   uv run python .github/workflows/scripts/lint_skill_refs.py
   python3 .github/workflows/scripts/lint_cursor_plugin.py --fix
   ```
...
5. Open a PR. CI validates SKILL.md frontmatter, MCP tool names, and `pipefy` CLI
   ...
   every published skill (`lint_cursor_plugin.py` in the `test` job, without
   `--fix`).
