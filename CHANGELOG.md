...
- **CLI**: `pipefy card fill <card_id> --phase <phase_id> --fields '{"…"}'` fills phase fields non-interactively (MCP `fill_card_phase_fields` with `skip_elicitation` equivalent); filters to editable phase field IDs before `update_card`. Optional `--required-only` limits the phase field lookup to required fields.
